...
function promoPoints() {
    ...
    const promoPointsSource = PropertiesService.getScriptProperties().getProperty("promoPointsSource");
...
function sendPointsMessage(message: string) {
    ...
    let slackWebhook = PropertiesService.getScriptProperties().getProperty("slackTestingWebhook");
...
// Slack boilerplate
function sendToSlack (url: string, payload: string) {
    var options: Object =  {
      "method" : "post",
